...
\textbf{TODO}: Explain that not everything has been implemented, focus on some transitions of TX and TD.
...
\textbf{TODO}: This list shouldn't appear as it is now.

Not mentioned in the thesis in order to keep its size reasonable and focus on the main topic:
\begin{itemize}
	\item Proof of gauss program not included because it doesn't introduce new results apart from the fact that multiplication in BitVector theory is a hard problem for SMT solvers.
	\item Git workflow for a team > crucial in order to build such a complex platform
	\item CI and tests to track regressions + static analysis > same than above
	\item Simple interface for CFG lib
	\item The formal NIC model is design with the idea that the simulation of the NIC would only consist of "while 1: model.autonomous\_step()" + 3 other function calls to represent interruptions from the CPU.
\end{itemize}

\section{Results}

> How to evaluate the work? + Give answer and evaluation.

> Present the novel value introduced in this thesis.
...
\begin{itemize}
    ...
    \item should change color of types (blue = free vars in HOL4 printer)
\end{itemize}

One-button proofs / non proof-producing verification library:
...
\begin{itemize}
    ...
    \item prove\_contract should take definitions instead of terms for P and Q, in order to be used in further proof or for composition.
    ...
    \item current limitations of the non pp lib: only simple linear programs; non pp; no composition.
    \item $Q_{BIR}$ isn't general because it doesn't take the initial BIR state.
\end{itemize}

Mention the "future" meeting that we had on the 29/05/2019.

Here is a paper proving sound satisfiability solver for bitvectors to check if the precondition entails the weakest precondition. We should do the same for arrays:
...
Proof:
\begin{itemize}
 	\item automate (???; semantic rewriting (i.e. automatically add all the lemmas)
 	\item > we must do the work at least once manually before automating procedures
 	\item doable and desirable in short-term: automatic procedure to turn the goal into a SMT-solvable goal (i.e. proof-producing BIR to words)
\end{itemize}

\section{Future work}

Some bits of the previous section.

%   -> identification (> DepGraph) ~et fragmentation difficile~
