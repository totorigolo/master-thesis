...
% - Git workflow for a team > crucial in order to build such a complex platform
% - CI and tests to track regressions + static analysis > same than above
% - Simple interface for CFG lib
...
\section{Pretty-printing to visualize BIR expressions}
...
\chapter{Tracing and error handling in HOL4} \label{annex_loglib}

\textbf{TODO}: Is this annex relevant in this thesis?

Reference in Section \ref{impl_convenient_ht_interface}, but the need arised in general when debugging HOL4 code for tracing code. LogLib represent an higher-level implementation of HOL4 tracing system. Verbosity levels are specified and output is colored, helping to read long outputs. Also include exception wrapping.

Content:
\begin{itemize}
	\item Present the problems specific to HOL4 that arise when debugging proofs and libs.
	\item Introduce tracing in HOL4 in general (levels, HOL4 warnings and error \textbf{messages}).
	\item Present logLib
	\item Introduce exception wrapping and explain how they help to locate the location of errors and can help interactive use of HOL4 by giving more precise information about what went wrong. Analogous to backtraces.
\end{itemize}
