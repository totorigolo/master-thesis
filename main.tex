...
HolBA's Binary Intermediate Representation (BIR) \cite{lindner_trabin:_2019} is a  machine-independent binary representation. It aims to be the simple while still being able to represent all possible binary programs but self-modifying programs\todo{too long sentence}. It does so by having a limited syntax---introduced in Table \ref{bir-syntax}---and forbidding implicit side-effects. A statement can only have explicit state changes and can only affect one variable.
...
execution speed rather than offline analysis.
...
\todo{I'm not sure that these examples with Alice and Bob are a good idea. These are probably too abstract}
